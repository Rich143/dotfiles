...
if [ -n "${FILE_LIST_CMD}" ]; then
    if [ "${PROJECT_ROOT}" = "." ]; then
        ...
        cat $DB_FILE.files | wc -l
    fi
else
    #find . -type f > "${DB_FILE}.files"
    git ls-files -- \*.c \*.cpp \*.h \*.hpp \*.cc > "${DB_FILE}.files"
fi
CSCOPE_ARGS="${CSCOPE_ARGS} -i ${DB_FILE}.files"

echo "Running cscope"
echo "$CSCOPE_EXE $CSCOPE_ARGS -q -b -k -f \"$DB_FILE.temp\""
"$CSCOPE_EXE" $CSCOPE_ARGS -v -b -k -q -f "$DB_FILE.temp" > cscope.log
...
mv -f "$DB_FILE.temp.in" "cscope.in.out"
mv -f "$DB_FILE.temp.po" "cscope.po.out"
